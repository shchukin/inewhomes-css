...
function symbolsImgToSpriteSvg(content) {
    ...
    source.forEach(function (line) {
        ...
        if( line.indexOf('symbols/') !== -1 ) {
            ...
            roleString   = line.match(        'role[ \t]*=[ \t]*"[^"]+"');
            ariaString   = line.match( 'aria-hidden[ \t]*=[ \t]*"[^"]+"');
            ...
            roleString   = roleString   ? roleString[0]   : null;
            ariaString   = ariaString   ? ariaString[0]   : null;
            ...
            outputLine[0] = indentString + '<svg' + ( classString ? ' ' + classString : '') + ( idString ? ' ' + idString : '') + ( widthString ? ' ' + widthString : '') + ( heightString ? ' ' + heightString : '') + ( titleString ? ' ' + titleString : '') + ( roleString ? ' ' + roleString : '') + ( ariaString ? ' ' + ariaString : '') + '>';
...
gulp.task('default',
  gulp.series('clean', 'manifest', 'favicon', 'index', 'temp', 'content', 'images', 'fonts', 'markups', 'layouts', 'vendors', 'scripts', 'symbols', 'styles', function (fn) {
    fn();
  })
);
